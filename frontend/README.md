...
안 넣으면 스크립트 요청이 401로 거부됩니다. (예전에는 [앱 설정] > [플랫폼]에도
별도로 도메인을 등록해야 했으나, 2026-08 기준 콘솔에서는 이 플랫폼 등록 메뉴
자체가 없어졌고 JavaScript 키 탭의 도메인 등록만으로 정상 동작을 확인했다.)
`.env.example`을 `.env`로 복사해 `VITE_KAKAO_MAP_KEY`에 발급받은 키를 설정하세요(`.env`는 커밋하지
