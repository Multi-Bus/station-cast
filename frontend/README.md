...
플랫폼에 로컬/배포 도메인 등록. `.env.example`을 `.env`로 복사해 `VITE_KAKAO_MAP_KEY`에
발급받은 키를 설정하세요(`.env`는 커밋하지 않습니다). **키가 없어도 앱은 기존 CSS
격자로 폴백 렌더링됩니다** -- 재현성 검증(외부인 clone 시나리오)이 카카오 키 발급
여부에 좌우되지 않도록 하기 위함입니다.
