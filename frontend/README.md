...
플랫폼에 로컬/배포 도메인 등록. **[앱 설정] > [앱 키] > JavaScript 키 탭에도 별도
도메인 등록란이 있는데, 플랫폼 등록만으로는 부족하고 여기에도 로컬/배포 도메인을
넣어야 합니다** -- 안 넣으면 스크립트 요청이 401로 거부됩니다. `.env.example`을
`.env`로 복사해 `VITE_KAKAO_MAP_KEY`에 발급받은 키를 설정하세요(`.env`는 커밋하지
않습니다). **키가 없어도 앱은 기존 CSS 격자로 폴백 렌더링됩니다** -- 재현성 검증
(외부인 clone 시나리오)이 카카오 키 발급 여부에 좌우되지 않도록 하기 위함입니다.
...
21개 정류장을 채웁니다. 백엔드가 응답하지 않으면(서버 미기동 등)
`src/data/mockStops.ts`의 `design-data.md` 샘플 데이터로 자동 폴백합니다 --
카카오 키 미설정 시 CSS 격자로 폴백하는 것과 같은 원칙으로, 백엔드 없이도
재현성 검증(외부인 clone 시나리오)이 가능하도록 하기 위함입니다.

백엔드에 아직 없는 값(사용자 위치 기반 거리, 지도·목록 화면의 정류장별 노선
목록, 환승거점 여부, 상세화면의 오늘 평균/전일 대비 통계, 대기인원 신뢰구간)은
목데이터·근사치로 채워 둡니다. 상세화면의 노선 목록만은 예외로, `/arrivals`
