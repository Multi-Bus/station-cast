...
2026 오픈소스 개발자대회 운영규정 제8조를 근거로, 라이선스를 두 층위로 구분해서 다룬다.
...
BSD 등"을 명시한다. 즉 GPL·LGPL은 규정상 허용된다. 이들을 프로젝트에서 배제하는 것은
Apache-2.0으로 배포하는 산출물과의 결합 이슈(라이선스 전염, 재배포 조건 충돌)를 피하려는
팀의 선택이지, 대회 규정이 요구하는 바는 아니다.
