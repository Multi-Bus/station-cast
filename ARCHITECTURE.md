...
명시하면 그 시간대의 과거 평균 패턴 값을 반환하고, 생략하면 "현재값"을
반환한다. `/congestion`은 `date`(생략 시 오늘)도 받아, 그 날짜의 요일·날씨·기온
보정계수를 평균값에 곱해 반환한다 — 보정계수를 구할 수 없으면 `weather_applied=false`로
평균값을 그대로 돌려준다. 지금은 이 "현재값"이 `_current_hour()`로 현재 시각을 그 시간대의 과거 평균
